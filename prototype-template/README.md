...
- **Metadata**: see `PROTOTYPE.yml` for the switch-off date and ownership
  details for this prototype.
- **What will and won't work here**: this is static hosting plus optional
  Netlify Functions — not a general-purpose server. Before asking Claude to
  add something like a persistent backend, websockets, or a database, see
  [`docs/PROTOTYPE-CONSTRAINTS.md`](https://github.com/publicdigital/claude-prototype-deploy/blob/main/docs/PROTOTYPE-CONSTRAINTS.md)
  in the `claude-prototype-deploy` repo for what's supported and what the
  alternatives are.
